...
#define LOG_ADD_FILE                                                 \
  boost::log::attribute_cast<                                        \
      ...
      ::boost::log::trivial::logger::get().get_attributes()["File"]) \
      .set(logger::path_to_filename(__FILE__))  // NOLINT
#define LOG_ADD_LINE                                                         \
  boost::log::attribute_cast<boost::log::attributes::mutable_constant<int>>( \
      ::boost::log::trivial::logger::get().get_attributes()["Line"])         \
      .set(__LINE__)  // NOLINT
...
#define LOG__(sev)                                                   \
  ...
  BOOST_LOG_STREAM_WITH_PARAMS(::boost::log::trivial::logger::get(), \
                               (::boost::log::keywords::severity =   \
                                    ::boost::log::trivial::sev))  // NOLINT

#define LOG_MSG(sev, msg) LOG__(sev) << msg
// NOLINTBEGIN
// Compile with -DMAX_LOG_LEVEL=info to statically deactivate all log messages below info.
#define LOG(sev)                                    \
  if constexpr (boost::log::trivial::sev >=         \
                boost::log::trivial::MAX_LOG_LEVEL) \
  LOG__(sev)
// NOLINTEND
namespace logger {
...
inline void init(boost::log::trivial::severity_level svl) {
  ...
  boost::log::trivial::logger::get().add_attribute(
      "File", attrs::mutable_constant<std::string>(""));
  boost::log::trivial::logger::get().add_attribute(
      "Line", attrs::mutable_constant<int>(0));
